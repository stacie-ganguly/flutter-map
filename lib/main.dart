...
class _MapPageState extends State<MapPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: MaplibreMap(
        styleString:
            'http://192.168.1.22:8000/styles/light.json?key=de261c3add45ec4b&mobile=true',
  ...
  Future<void> _onMapCreated(MaplibreMapController controller) async {
    ...
    final routeUrl = 'http://192.168.1.22:8000/geojson/routes.json';
    ...
    final startMarkerUrl =
        'http://192.168.1.22:8000/geojson/start-markers.json';
  ...
  double _distancePointToLineSegment(
    ...
    for (final feature in _routesGeoJson!['features']) {
      ...
      if (geometry['type'] == 'LineString') {
        ...
        for (int i = 0; i < coords.length - 1; i++) {
          ...
          double dist = _distancePointToLineSegment(coordinates, startPoint, endPoint);
